...
export function AppHeader({ agentRunning, cardCount, onOpenSettings, onRun, projectTitle, saveState }: AppHeaderProps) {
  return <header className="topbar">
    ...
    <div className="project-title"><span>{projectTitle}</span><small className={`header-save-state ${saveState}`}>{saveState === 'recovering' ? 'Recovery available' : saveState === 'saved' ? 'Saved' : `Unsaved${cardCount === 0 ? ' · empty canvas' : ''}`}</small></div>
