...
export const scenarioPresets: Record<ScenarioPresetId, ScenarioPreset> = {
  ...
  'schema-drift': {
    ...
    nodes: [
      ...
      { id: 'drift-contract', type: 'pipeline', position: { x: 665, y: 180 }, data: { kind: 'validation', label: 'Feature schema contract', description: 'The feature pipeline still requires numeric customer_age.', owner: 'ML Platform', status: 'blocked', schema: [], rule: 'schema_contract: customer_id:string, customer_age:number' } },
      { id: 'drift-output', type: 'pipeline', position: { x: 985, y: 180 }, data: { kind: 'output', label: 'churn_prediction_v3', description: 'Production model deployment at high risk until age_bucket is repaired and the model is retrained.', owner: 'ML Platform', status: 'blocked', schema: [] } },
    ],
    edges: [
      { id: 'e-drift-impact', source: 'drift-source', target: 'impact-lineage', type: 'elastic' },
      { id: 'e-drift-contract', source: 'impact-lineage', target: 'drift-contract', type: 'elastic' },
