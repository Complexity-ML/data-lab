const defaultBlankObjective = 'Start an evidence-backed autonomous incident workflow. Use the single adjustable host-owned Catalog Explorer sidecar to audit its configured governed dataset or catalog in bounded parallel batches, then select and branch from supported evidence without silently dropping catalog assets. Never connect Catalog Explorer to dataset lineage: it updates only its own checkpoint and Data Source branches start independently. Use Query Check nodes inside a branch to verify registered GraphQL reads or governed writes; never place arbitrary query text in a card, and route governed writes through Human Review. If a verified query exposes an incompatibility, use a reversible graph-only Compatibility Patch, recheck it, assess risk, then request review before any governed execution. Propose one coherent useful iteration with every card and connection required for that stage, commit the complete diff as a restorable version, then let the player reread the resulting graph and fresh evidence before continuing. Grow incrementally toward DATA LAB Control, Data Source, Query Check, Live Monitor, compact Data Profile, Impact Analysis, Risk Assessment, Agent Decision, Validation and Output with feedback; add Human Review only for uncertainty, governed writes or high-risk impact. When the connector is unavailable, create only an unbound Data Source plus Human Review and never invent metadata.'
