  ...
  return <>
    ...
    <div className="reports-panel-content">
      ...
      {active.length ? <div className="reports-list">{active.map((incident) => <button className={`severity-${incident.severity} status-${incident.status}`} disabled={!incident.cardId} key={incident.incidentKey} onClick={() => incident.cardId && onSelectCard(incident.cardId)} type="button">
        ...
        <div><strong>{incident.title}</strong><p>{incident.detail}</p><small>{incident.status.replace('-', ' ')} · {incident.occurrenceCount} occurrence{incident.occurrenceCount === 1 ? '' : 's'} · {new Date(incident.updatedAt).toLocaleString()}</small></div>
      </button>)}</div> : <div className="reports-clear"><CheckCircle2 size={18} /><span><strong>No unresolved incident</strong><small>Live Monitor reports requiring attention will appear here.</small></span></div>}
