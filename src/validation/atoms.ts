...
export const dataHubGovernanceAtom: ValidationAtom = {
  ...
  run({ nodes }) {
    return nodes.flatMap((node) => {
      ...
      if (node.data.datahubFreshness?.stale) findings.push(issue(this.id, { id: `metadata-stale-${node.id}`, severity: sensitive ? 'error' : 'warning', nodeId: node.id, title: 'Catalog evidence is stale', detail: sensitive ? 'Sensitive-data evidence expired, so the agent cannot proceed until connector context is refreshed.' : 'Refresh catalog evidence before relying on this metadata.' }))
      return findings
    })
  },
}
