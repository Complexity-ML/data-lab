...
describe('SQLite workspace persistence', () => {
  ...
  it('migrates v1 workspace, draft, versions and checkpoints without retaining JSON blob columns', () => {
    ...
    const path = join(target, 'data-lab.sqlite')
    ...
    `)
    const nodes = [
      { id: 'source', type: 'pipeline', position: { x: 10, y: 20 }, data: { kind: 'source', label: 'Orders', schema: [{ name: 'order_id', type: 'string' }] } },
      { id: 'profile', type: 'pipeline', position: { x: 310, y: 20 }, data: { kind: 'profile', label: 'Orders profile', schema: [] } },
    ]
    const edges = [{ id: 'source-profile', source: 'source', target: 'profile', type: 'elastic' }]
    const committed = {
      projectTitle: 'Order governance',
      nodes,
      edges,
      versions: [{
        id: 'version-1',
    ...
    const draft = { ...committed, projectTitle: 'Order governance draft', nodes: [...nodes, { id: 'review', type: 'pipeline', position: { x: 610, y: 20 }, data: { kind: 'review', label: 'Review', schema: [] } }] }
    legacy.prepare(`
      INSERT INTO workspaces (id, name, payload, draft_payload, archived, dirty, created_at, updated_at, draft_updated_at)
      VALUES ('workspace-v1', 'Order governance', ?, ?, 0, 1, ?, ?, ?)
    ...
    legacy.prepare('INSERT INTO catalog_checkpoints (scope_id, checkpoint_key, payload, updated_at) VALUES (?, ?, ?, ?)').run(
      'workspace-v1',
      'catalog:test',
      JSON.stringify({ inspected: 2, total: 2, datasets: [{ urn: 'urn:orders', issues: [] }] }),
    ...
    expect(loadCatalogCheckpoint(target, 'catalog:test')).toEqual({ inspected: 2, total: 2, datasets: [{ urn: 'urn:orders', issues: [] }] })
    closeWorkspaceDatabase()

    expect(existsSync(join(target, 'data-lab.pre-relational-v1.sqlite'))).toBe(true)
