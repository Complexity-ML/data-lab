...
export function parseCatalogConnectorManifest(value: unknown): CatalogConnectorManifest {
  ...
  const contract = input.contract === undefined || input.contract === 'data-lab.catalog.v1' ? 'data-lab.catalog.v1' : undefined
  ...
  if (!kind || !contract) throw new Error('Only the data-lab.catalog.v1 MCP or HTTP API contract is supported')
...
async function withMcp<T>(connector: CatalogConnectorManifest, operation: (client: Client) => Promise<T>) {
  ...
  const client = new Client({ name: 'data-lab-catalog', version: app.getVersion() })
  try {
...
export async function testCatalogConnector(idValue: unknown) {
  ...
  if (connector.kind === 'http-api') {
    ...
    return { connected: true, message: `${connector.name} implements data-lab.catalog.v1` }
  }
