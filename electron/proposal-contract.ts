...
export type ProposalCardKind = 'source' | 'profile' | 'analysis' | 'split' | 'decision' | 'transform' | 'review' | 'validation' | 'output'
...
const kinds = new Set<ProposalCardKind>(['source', 'profile', 'analysis', 'split', 'decision', 'transform', 'review', 'validation', 'output'])
const actionTypes = new Set<ProposalActionType>(['add_card', 'update_card', 'add_edge', 'remove_edge'])
const cardNames: Record<ProposalCardKind, string> = { source: 'Data Source', profile: 'Data Profile', analysis: 'Data Analysis', split: 'Split', decision: 'Agent Decision', transform: 'Transform', review: 'Human Review', validation: 'Validation', output: 'Output' }
