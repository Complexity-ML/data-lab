  ...
  for (let turn = 0; turn < 24; turn += 1) {
    const response = await authorizedFetch('openai', '/responses', {
      ...
      body: JSON.stringify({
        ...
        instructions: `${activeInstructions} Use the provided DATA LAB tools. Call list_card_kinds before queuing changes, call validate_plan, then finish_plan exactly once. Tools never execute the graph.`,
        input,
